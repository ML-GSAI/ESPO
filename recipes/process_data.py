# Adapted from Diffucoder: https://github.com/apple/ml-diffucoder/blob/main/recipes/process_data.py
...
def main(
    ...
    if difficulty in ["easy", "medium", "hard"]:
        ...
        def get_accs(item):
            ...
            item['accs'] = accs
            return item
        dataset = dataset.map(get_accs, desc="Getting accs", num_proc=64)
        ...
        if difficulty == "hard":
            ...
            dataset = dataset.filter(lambda x: np.std(x['accs']) >= split_std, desc="Filtering high std examples" ,num_proc = 64)
            ...
            dataset = dataset.filter(lambda x: np.mean(x['accs']) >= split_acc, desc="Filtering high average examples" ,num_proc = 64)
